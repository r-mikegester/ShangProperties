...
class Media {
    ...
    constructor({
        ...
        route,
        navigate,
    }: MediaProps & { route: string; navigate?: (path: string) => void }) {
      ...
      `,
        ...
        void main() {
          ...
          vec2 uv = vec2(
            scaledUv.x * ratio.x + (1.0 - ratio.x) * 0.5,
            scaledUv.y * ratio.y + (1.0 - ratio.y) * 0.5
          );
...
class App {
    ...
    planeGeometry!: Plane;
    medias: Media[] = [];
    mediasImages: { image: string; text: string; route: string }[] = [];
    ...
    navigate?: (path: string) => void;
        ...
        const mouseY = -((event.clientY - rect.top) / rect.height) * 2 + 1;
            ...
            if (
                ...
                if (media.navigate && media.route) {
                    media.navigate(media.route);
                }
                break;
            }
        }
    ...
    hoveredMediaIndex: number | null = null;
    ...
    handleMouseMove(event: MouseEvent) {
        ...
        for (let i = 0; i < this.medias.length; i++) {
            ...
            if (
                ...
                if (this.hoveredMediaIndex !== i) {
                    if (this.hoveredMediaIndex !== null) {
                        this.medias[this.hoveredMediaIndex].setHovered(false);
                    }
                    media.setHovered(true);
                    this.hoveredMediaIndex = i;
                }
                found = true;
                break;
            }
        }
        if (!found && this.hoveredMediaIndex !== null) {
            this.medias[this.hoveredMediaIndex].setHovered(false);
            this.hoveredMediaIndex = null;
        }
    }
    ...
    createMedias(
        items: { image: string; text: string; route: string; }[] | undefined,
        bend: number = 1,
        textColor: string,
        borderRadius: number,
        font: string
        ...
        const defaultItems = [
            ...
            {
                image: WackWack,
                text: "Shang Residences at Wack Wack",
                route: "/WackWack",
            },
        ];
        ...
        this.medias = this.mediasImages.map((data, index) => {
            const media = new Media({
                ...
                borderRadius,
                font,
                route: data.route, // Pass the route here
                navigate: this.navigate, // Pass navigate
            });
